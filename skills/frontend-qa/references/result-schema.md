...
`professionalAudit` is the report-contract self-audit available as `professional-audit.md`, `professional-audit.json`, and `frontlens audit --report`. Read it before trusting a report produced by another Agent. A `failed` audit means the report is internally inconsistent or overclaims: for example it schedules non-proof-ready/non-actionable items as fixes, allows a business-validation claim without requirements/assertions, overstates acceptance while qaCoverage has gaps, has failed artifact integrity, or lacks required source evidence for a frontend fix.
