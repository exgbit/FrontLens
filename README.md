...
- `defectProof.status!=ready` 或某个 root cause 为 `needs-evidence` 时，不能直接把它当 must-fix 缺陷排期；先补齐缺失证据或降级为条件项/观察项。
