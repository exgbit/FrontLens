...
```bash
node dist/cli.js test-plan --input /path/to/prd.md --source-root /path/to/service \
  --project-type backend --output reports/backend-plan
```

后端模式不会生成 frontend/browser 用例，可用性、认证和核心流程阻塞项落到 API/服务层。`--project-type auto` 只读取有界的项目元数据和框架标记自动识别；识别不确定时可由 Skill 在源码复核后显式选择 `backend`。
