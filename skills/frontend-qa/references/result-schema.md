...
`qaPlan` is the professional tester worklist. It sits above raw issues and regression details: it tells the next tester/Agent which requirements, journeys, product-context decisions, environment checks, test data, proof-ready fixes, and evidence gaps must be handled before acceptance or release claims.
