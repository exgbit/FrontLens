...
Report raw score separately from adjusted risk, `riskRegister.status`, and `riskAcceptance.status`. If many findings are skipped, synthetic, or deployment-only, say the score is low-confidence and prioritize the triaged fix list instead of repeating every issue.

Use `qaSignoff`, `riskRegister`, `environment`, and `pageProfile` before final sign-off wording. If `qaSignoff.businessValidationConfidence` is not `runtime-verified`, or `environment.trust.performance/security` is not high for release claims, do not describe business validation or production readiness as fully passed even when `qualityGate.status=pass`.

Use `riskRegister` and `riskAcceptance` before release-style prioritization: if `riskRegister.status=blocked` or `riskAcceptance.status=blocked`, report release-blocking/must-mitigate risks and their owners/mitigations before raw issue totals. If `riskAcceptance.status=needs-acceptance`, list the required approvers/evidence instead of calling the risk fixed. Use `defectProof` before scheduling fixes: if a root cause is `needs-evidence`, list the missing evidence and do not call it must-fix until confirmed. In FrontLens 1.78+, use `defectTickets` / `defect-tickets.md` as the Jira/Linear filing queue; it should contain only proven/probable root causes, so anything missing from it is not ready for implementation ticketing unless a human deliberately overrides the proof gate. In FrontLens 1.79+, use `traceability` / `traceability.md` before business validation wording; high-priority requirement gaps or orphan proof-ready defects mean the conclusion must stay conditional. In FrontLens 1.80+, use `automationSpecs` / `automation-specs.md` / `automation/frontlens.spec.ts` as a draft regression asset only; do not count generated specs as passed evidence until reviewed and executed. In FrontLens 1.81+, use `evidenceBundle` / `evidence-bundle.md` as the handoff proof index; in FrontLens 1.82+, use `qaStrategy` / `test-strategy.md` to decide run/defer/out-of-scope/block strategy. `missing-artifact` items block citing paths as evidence. Use `claimGuard` before final wording: if `claimGuard.status=limited|blocked`, remove or explicitly negate every `forbiddenClaims[]` phrase and use `items[].allowedWording` instead. Use `qaIntake` before turning uncertainty into conclusions: if `qaIntake.status=needs-input|blocked`, list `topQuestions[]` and keep linked claims conditional until answered. Use `scopeReview` before product/design conclusions. If `scopeReview.status=needs-input`, answer the questions in `scope-review.md` or copy the confirmed `configSnippet.productContext` into the next run before promoting style, pagination, export, refresh, responsive, visual-density, or device-scope findings to must-fix defects.
...
- Move to **reference/product decision** instead of fixes: visual density, color/style preference, number of primary buttons, manual refresh/export/pagination expectations, optional SEO on admin pages, small tap targets on PC-first products, or any feature that depends on product requirements. In FrontLens 1.43+, warning-only refresh/download/pagination probes without explicit PRD/productContext requirements should normally appear only under interaction coverage, not the raw issue work queue. In FrontLens 1.44+, small tap-target observations without explicit mobile/touch/WCAG scope should appear only under responsive coverage. In FrontLens 1.45+, treat adjusted score and proof-ready root causes as the report headline; raw score is scanner trend context. In FrontLens 1.46+, optional SEO gaps and color-contrast findings need explicit public-content/SEO/WCAG/a11y scope before becoming fixes. In FrontLens 1.47+, category-mismatched API/table/pagination repair advice on a11y/SEO/visual findings is sanitized automatically. In FrontLens 1.48+, keep raw Vite dev resource metrics as noise, but keep sourceAnalysis file:line route/static-import findings as source-level should-fix candidates. In FrontLens 1.49+, any Markdown path marked `(missing artifact)` or `(unchecked artifact)` is not valid proof until artifactIntegrity passes or the artifact is regenerated. In FrontLens 1.50+, read the `qa-review.md` downgraded/non-fix samples before raw issue drill-down; in FrontLens 1.64+, read `risk-register.md/json` before release summaries because it separates implementation defects from coverage/environment/source/test-data/artifact risks; they are the preferred concise explanation for why style/product/tool/deployment/no-evidence findings are not implementation tasks.
...
- For API/UI data-mismatch claims, require the four-part data-binding proof gate before calling it a real frontend defect: explicit product/PRD requirement, exact list-like `networkRequestId` with response path/count, visible empty DOM/screenshot state for the target table/list/card region, and source API/state/rendering file:line or medium/high `sourceRuntimeCorrelation`. Missing any layer means **conditional / insufficient evidence**, not a fix; assign follow-up to QA/test evidence collection. In FrontLens 1.42+, when sourceRoot/sourceAnalysis is enabled, list-empty mismatch guesses with unavailable, none, or low sourceRuntimeCorrelation are intentionally suppressed before raw issue creation; in FrontLens 1.69+, even emitted data-mismatch findings remain non-actionable until all four proof layers are present. In FrontLens 1.75+, raw data-mismatch creation itself requires an explicit provided requirement plus medium/high sourceRuntimeCorrelation; missing either is a QA intake/source-binding gap, not an implementation ticket. In FrontLens 1.70+, weak empty-state/pagination observations should remain QA/test evidence gaps unless PRD/productContext makes them required. In FrontLens 1.71+, generic body/html/#app text assertions such as `OK` or very short success text are weak, not runtime-verified business proof. In FrontLens 1.72+, missing PRD/acceptance criteria or unconfirmed product/page scope should lead the human-facing summary as QA intake needed, not as a defect list. In FrontLens 1.73+, keep final summaries no deeper than the compact brief unless the user explicitly requests evidence-detail expansion.
...
- Count implementation work by proof-ready root cause, not by raw issue IDs. `fixTasks[]` is proof-aware in FrontLens 1.32+ but remains machine-oriented; `defectProof.needs-evidence` items are evidence-collection work, not implementation defects. In FrontLens 1.34+, use `rootCauseGroups[].sourceLocations` to report the normalized file:line fix surface instead of re-deriving it from each raw issue; in FrontLens 1.36+ this field also includes medium/high source-runtime links, in FrontLens 1.37+ it can include source ui-accessibility findings for runtime a11y roots, in FrontLens 1.38+ it can include source error-state-gap findings for exception no-feedback roots, in FrontLens 1.39+ those error-state findings cover Vue/Svelte/JSX pages, in FrontLens 1.40+ the a11y findings include multi-line icon-button tags, in FrontLens 1.41+ built-in heuristic AI summaries no longer create raw issues, in FrontLens 1.42+ source-aware list-empty mismatch guesses require medium/high sourceRuntimeCorrelation before raw issue creation, in FrontLens 1.43+ product-scoped refresh/download/pagination warnings remain interaction coverage observations unless explicitly required, in FrontLens 1.44+ small tap-target observations remain responsive coverage unless mobile/touch/WCAG scope is explicit, in FrontLens 1.45+ professional reports lead with adjusted score/proof-ready root causes instead of raw score, in FrontLens 1.46+ optional SEO gaps/color-contrast findings stay product-scope observations unless explicit scope requires them, in FrontLens 1.47+ mismatched template suggestions are stripped from fix guidance, in FrontLens 1.48+ sourceAnalysis-confirmed eager route/heavy static imports remain proof-ready source-level should-fix items even when dev resource metrics are downgraded, in FrontLens 1.49+ missing/unchecked artifact paths cannot be cited as evidence, in FrontLens 1.50+ `qa-review.md` includes representative non-fix samples with reasons and next steps, and weak/missing source-bound frontend root causes stay needs-evidence, and in FrontLens 1.64+ release-blocking work should be summarized from `riskRegister.items[blocksRelease=true]` before raw issue IDs, in FrontLens 1.65+ must-mitigate release work should be summarized from `riskAcceptance.items[decision=must-mitigate]` before accepted/acceptance-needed risks, in FrontLens 1.75+ `suggestions`/MCP suggestions are proof-aware by default, in FrontLens 1.78+ `defectTickets.items[]` is the preferred human bug-filing list, and in FrontLens 1.79+ `traceability.requirements[]` is the preferred business sign-off mapping; in FrontLens 1.80+ `automationSpecs.drafts[]` is the preferred review-only Playwright starter for regression coverage, and in FrontLens 1.81+ `evidenceBundle.items[]` is the preferred proof-handoff checklist, and in FrontLens 1.82+ `qaStrategy.modules[]` is the preferred run/defer/out-of-scope/block strategy checklist; in FrontLens 1.83+ `businessJourneys.scenarios[]` is the preferred requirement/journey/assertion/role/test-data scenario plan; in FrontLens 1.84+ `reviewCalibration.issueDecisions[]` is the preferred human-feedback calibration before filing style/product/dev/data-mismatch findings, in FrontLens 1.85+ `reviewCalibration.calibrationSource=config` means a previous review-calibration config is actively applied, and in FrontLens 1.86+ that applied config also drives `issueDisposition`/adjusted score/fixTasks while explicit current-page required scope still overrides the old calibration; use `--all`/`all=true` only for raw audit.
...
- If a generated suggestion does not match the issue category or evidence, mark it as template noise and replace it with a source/evidence-specific fix; in FrontLens 1.47+ this is built into suggestion templates for common API/table/pagination noise on a11y/SEO/visual findings. Treat `aiAnalysis.summary/suggestions` from the built-in heuristic provider as advisory context only; never count it as an implementation defect unless an underlying runtime/source/proof-ready root cause supports it.
...
Use `businessJourneys.status` as a planning signal only: `ready` means actions/assertions are available to rerun, while `needs-input` and `manual-required` are QA/product/test-data follow-ups, not implementation defects.
...
- `deviceScope`: downgrade mobile/touch-target findings for `desktop-only` / `desktop-first`; keep them in scope for `mobile-first`, `responsive`, and stricter accessibility targets. In 1.44+, lack of explicit mobile/touch/WCAG scope keeps small tap-target counts as coverage rather than raw defects.
...
If `result.json.reviewCalibration` exists, use it as the first reusable feedback contract after `issueDisposition`: `issueDecisions[action=keep]` can stay in the candidate defect list only if defectProof/source/runtime evidence also supports it; `downgrade`, `out-of-scope`, `needs-evidence`, and `ask-product` must not be reported as must-fix. If `reviewCalibration.status=needs-feedback`, explicitly ask for tester/Product feedback instead of guessing product intent. If `reviewCalibration.calibrationSource=config`, treat the previous human review as already applied to machine disposition/score/fix queues and do not ask the same scope question again unless the page type/requirements changed. If the current page explicitly requires a feature that old calibration marked optional/out-of-scope, follow the current requirement and keep the finding in scope. When feedback says “样式设计如此 / PC-first / dev server 噪音 / 接口有数据页面空误报”, preserve those decisions in `review-calibration.config.json` and rerun with `--config` so the calibration is portable to future same-type pages.
...
3. **Table/list heuristic mismatch**
   ...
   - If a scanner says "API has data but table/page is empty", keep it only when the exact API response is a list-like object array (`records`/`rows`/`list`/`items`/`results`, or `data` from a list-like endpoint), the visible DOM for the target table/list/card is genuinely empty, explicit requirement evidence says that data must render there, and source code or E2E evidence binds that API to that UI. When `sourceRuntimeCorrelation.status=passed`, require the relevant link `confidence` to be `medium/high`; otherwise classify as insufficient evidence or false positive.
...
8. **Artifact links and copied reports**
   ...
   - If a referenced artifact is missing, mark it as a tool/reporting issue and do not use it as evidence. In FrontLens 1.35+, prefer `result.json.artifactIntegrity` plus the rewritten `report.md`/`report.html`; if they disagree, trust result.json and report a reporter-staleness bug. In FrontLens 1.36+, when sourceRoot is enabled, do not schedule a frontend fix whose defectProof remains needs-evidence because source binding is weak/missing. In FrontLens 1.81+, also check `evidenceBundle`; in FrontLens 1.82+, also check `qaStrategy`; in FrontLens 1.84+, also check `reviewCalibration` before promoting product/style/dev/data-mismatch findings, in 1.85+ honor `calibrationSource=config` as an applied rerun contract, and in 1.86+ prefer the calibrated `issueDisposition`/score/fix queue unless current explicit requirements override it. If a ticket/test/gap references `artifactRefs[].exists=false`, ask QA to regenerate artifacts before citing that evidence.
...
- API/UI binding: if `sourceRuntimeCorrelation.links[]` exists, cite the specific link id and confidence. Do not retain “接口有数据但页面空” as a defect when requirement, network, UI-empty, or source-binding proof is missing, or when the link is missing/`confidence=none/low`.
...
1. Professional headline: QA sign-off, riskRegister/riskAcceptance status, release-blocking/must-mitigate count, adjusted score, proof-ready root-cause count, and confidence.
2. Top real fixes, ordered by P1/P2/P3 and grouped by root cause.
3. Non-defect buckets: product/ADR tradeoffs, deployment/security config, tool limitations, and insufficient evidence.
4. Raw artifact paths and raw score as reference/trend context only.
