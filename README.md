...
专业 QA 结论里，导出/下载只有在文件真实存在、大小非 0、哈希已记录、且 `artifactIntegrity` 通过时才算 `runtime-verified`。只有网络请求、没有文件产物时，只能算 `runtime-partial`。
