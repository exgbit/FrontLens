...
1. **Scope and assumptions**: target URL, route/page, sourceRoot, environment, pageProfile/scopeReview/product scope, reviewCalibration feedback contract, claimGuard wording limits, qaIntake top questions, defectProof proof gaps, auth role, selected modules, allowed/destructive actions, known missing inputs.
...
4. **Defect triage and tickets**: core defects by root cause, raw-finding disposition, severity, owner, reproduction, fix surface, merged raw issue IDs, and `defectTickets` for Jira/Linear-ready filing. Do not list every raw issue as a separate bug.
5. **Non-defect observations**: product decisions, style/design suggestions, skipped checks, environment/deployment tasks.
6. **Assertion and business-scenario improvement pack**: summarize `result.json.assertionSuggestions` / `assertion-suggestions.md`, `result.json.businessJourneys` / `business-journeys.md/json`, `result.json.reviewCalibration` / `review-calibration.md/json`, and the review-only `qa-intake.config.json._frontlensQaIntake.draftAssertionSteps[]`, especially concrete expect* suggestions plus requirement/journey/role/test-data scenarios for path-only or weak journeys. Make clear these are draft assertions/scenarios to add and rerun, not passed evidence.
...
9. **Automation and regression pack**: include `result.json.automationSpecs` / `automation-specs.md` / `automation/frontlens.spec.ts` as review-only Playwright drafts and `result.json.evidenceBundle` / `result.json.qaStrategy` / `evidence-bundle.md` / `test-strategy.md` as the proof handoff manifest, then use `result.json.qaPlan` first as the professional tester worklist, then `result.json.regressionPlan` for detailed repair verification; include exact FrontLens rerun commands, blocked/needs-input items, journey/requirement/download/environment checks, and focused verification steps after fixes.
10. **Sign-off status**: one of `pass`, `pass-with-risks`, `blocked`, or `fail`, with confidence (`high`, `medium`, `low`) and explicit blockers.

Use `result.json.professionalSummary` as the first human-facing triage summary, `professional-audit.md/json` as the report-contract, disposition-quality, and coverage-boundary self-check, `report-content-audit.md/json` as the generated Markdown wording/depth self-check, `result.json.qaIntake` / `qa-intake.md` / `qa-intake.config.json` as the professional follow-up question and rerun-input pack, `journey-assertion-audit.md/json` as the business-flow assertion-quality self-check, `result.json.businessJourneys` / `business-journeys.md/json` as the scenario planning pack, `result.json.reviewCalibration` / `review-calibration.md/json/config` as the human-feedback calibration pack, `result.json.qaPlan` as the professional execution/acceptance worklist, `result.json.qaCoverage` as the coverage boundary, `result.json.testCases` as the formal test-case execution matrix, `result.json.riskRegister` as the release risk register, `result.json.riskAcceptance` as the risk-acceptance/must-mitigate gate, `result.json.defectTickets` / `defect-tickets.md/json` as the bug-filing queue, `result.json.traceability` / `traceability.md/json` as the PRD-to-test-to-defect matrix, `result.json.automationSpecs` / `automation-specs.md/json` / `automation/frontlens.spec.ts` as the review-only automation draft package, `result.json.evidenceBundle` / `result.json.qaStrategy` / `evidence-bundle.md/json` + `test-strategy.md/json` as the evidence handoff package, `result.json.claimGuard` as the anti-overclaim wording gate, `result.json.defectProof` as the root-cause proof-strength gate, and `result.json.qaSignoff` as the first machine-readable professional sign-off, then inspect `regressionPlan`, `qualityGate`, `requirementCoverage`, `environment`, `pageProfile`, `scopeReview`, `claimGuard`, `defectProof`, `defectTickets`, `traceability`, `automationSpecs`, `evidenceBundle`, `qaStrategy`, `businessJourneys`, `riskRegister`, `riskAcceptance`, `reportContentAudit`, `journeyAssertionAudit`, `sourceAnalysis`, `sourceRuntimeCorrelation`, `sourceHealth`, `artifactIntegrity`, `issueDisposition`, and `rootCauseGroups` for the supporting evidence. For example, a raw `qualityGate.pass` can still be `qaSignoff.pass-with-risks` when requirements, role, test data, non-production environment, scope questions, or relevant journeys are missing.
...
- Product scope/ADR context. Inspect `pageProfile`, `scopeReview`, and `reviewCalibration`; if scope is inferred or `scopeReview.status=needs-input`, answer its questions before promoting style/product findings. Encode supported devices, page type, required/optional/out-of-scope features, and ADR references as `productContext` so intentional design choices do not become defects.
...
- **Business journeys**: inspect `businessJourneys.status`, ready/needs-input/manual-required counts, `scenarios[]`, role needs, test-data needs, and gaps. Use it as the consolidated scenario plan across PRD requirements, recorded journeys, assertion drafts, and manual discovery; do not file manual-required/needs-input rows as code defects.
- **Review calibration**: inspect `reviewCalibration.status`, `reviewCalibration.calibrationSource`, `signals[]`, `issueDecisions[]`, and `configPatch`. If it is `ready`, apply keep/downgrade/out-of-scope/needs-evidence/ask-product decisions before the fix list; if `calibrationSource=config`, state that a prior human feedback config is already applied, and in schema 1.86+ use the calibrated `issueDisposition`/score/fix queue unless current page requirements override it; if it is `needs-feedback`, ask for tester/Product feedback rather than inventing product intent.
...
- **Defect tickets**: inspect `defectTickets.status`, `counts.total`, `counts.suppressedNeedsEvidence`, and `items[]`. Use `defect-tickets.md/json` for bug filing; if an observation is absent from this queue, it needs more proof or is a non-defect bucket unless a human explicitly overrides.
- **Traceability**: inspect `traceability.status`, `summary.highPriorityGapCount`, `requirements[]`, and `orphanItems[]`. Do not claim business validation when PRD requirements are missing, high-priority requirements are failed/not-covered, or proof-ready defects are orphaned from requirements.
- **Automation specs**: inspect `automationSpecs.status`, ready/needs-input/blocked counts, `drafts[]`, and `automation/frontlens.spec.ts`. Treat them as review-only Playwright starters; they become evidence only after QA reviews selectors/auth/test data/safety and runs them.
- **Evidence bundle**: inspect `evidenceBundle.status`, `summary.missingArtifactCount`, `items[]`, and `artifactRefs[]`; do not cite any local artifact where `exists=false`.
- **Test strategy**: inspect `qaStrategy.status`, `summary.riskLevel`, `summary.recommendedRunMode`, module decisions, environment plan, and questions; use it to avoid over-testing out-of-scope product/style choices or trusting dev-server performance/security noise.
...
- If there are no proof-ready defects, say that explicitly and provide the missing inputs needed to turn conditional observations into defects. In FrontLens 1.72+, prefer the intake-first `professionalSummary.headline` when PRD/product scope is missing. In FrontLens 1.73+, treat `brief.md` as the maximum default answer shape; do not expand selector/module detail unless the user asks. In FrontLens 1.74+, prefer `qa-intake.config.json` as the editable answer sheet for missing PRD/product/test-data/source inputs before rerun. In FrontLens 1.75+, use proof-aware `suggestions` for implementation guidance and `suggestions --all` only for auditing suppressed raw suggestions. In FrontLens 1.76+, use `claim-guard`, `defect-proof`, `report-content-audit`, and `journey-assertion-audit` helpers before writing final conclusions or fix queues. In FrontLens 1.78+, use `defect-tickets` for the human bug-filing queue. In FrontLens 1.79+, use `traceability` before business-validation wording. In FrontLens 1.80+, include `automationSpecs` when the user wants a professional-tester replacement or regression pack, but label generated specs as draft until executed. In FrontLens 1.81+, include `evidenceBundle` when handing results to engineers/release reviewers; in FrontLens 1.82+, also include `qaStrategy` for run/defer/out-of-scope/block decisions; in FrontLens 1.83+, include `businessJourneys` as the professional scenario plan and keep it separate from passed evidence. Blocked/missing-artifact means the evidence package must be regenerated or corrected before citing paths.
...
- No business pass from a recorded journey that only contains `click`/`fill`/`press`; add `expectVisible`/`expectText`/`expectUrl`/`expectRequest` or requirement evidence first. `assertionSuggestions` and `businessJourneys` can propose candidates/scenarios, but they are not evidence until executed.
...
- No product/design/style/API-data mismatch uncertainty should be promoted to a must-fix bug while `qaIntake.topQuestions[]` blocks the linked claim or `reviewCalibration.issueDecisions[]` marks it downgrade/out-of-scope/needs-evidence/ask-product.
...
- No Jira/Linear bug should be filed from raw issues alone when `defectTickets.items[]` is empty; use `defectProof.missingEvidence` and `qaIntake.topQuestions[]` to collect proof first.
...
## Business journeys
- Status: ready | needs-input | manual-required | skipped
- Scenarios / ready / needs-input / manual-required: ...
| ID | Priority | Source | Status | Requirements | Assertions | Role/test-data gap | Next step |
| --- | --- | --- | --- | --- | --- | --- | --- |
...
- Business journeys: businessJourneys.status, scenario/ready/needs-input/manual-required counts
