...
test('guardrail result commands expose professional QA gates', async () => {
  ...
  assert.match(runCli('review-calibration', reportPath), /FrontLens Review Calibration/);
  ...
  assert.ok(['ready', 'needs-feedback', 'needs-input'].includes(JSON.parse(runCli('review-calibration', reportPath, ['--json'])).status));
