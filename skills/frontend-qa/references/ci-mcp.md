...
- `frontlens_review_calibration`: return reviewer/Product/QA feedback calibration, `calibrationSource` (`none|feedback|config`), issue actions, and rerun config patch for style/design/dev-server/data-mismatch false-positive reduction.
...
- `frontlens_traceability`: return PRD-to-test-to-defect traceability; use it before business validation or release sign-off wording.
- `frontlens_automation_specs`: return review-only Playwright automation drafts generated from requirements, journeys, assertion suggestions, and test cases.
- `frontlens_evidence_bundle`: return the shareable evidence handoff manifest and flag missing local artifact refs before engineers/release reviewers cite screenshots/videos/downloads.
...
- `frontlens_business_journeys`: return requirement/journey/assertion/role/test-data business scenarios for rerun planning; not pass evidence until executed.
...
- `$OUTPUT_DIR/automation-specs.md` and `$OUTPUT_DIR/automation/frontlens.spec.ts`
- `$OUTPUT_DIR/evidence-bundle.md` / `$OUTPUT_DIR/evidence-bundle.json`
