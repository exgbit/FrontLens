...
- API Contract, Realtime, Performance, Coverage, P2 tests, Security, Requirement Coverage, Environment, Page Profile, Scope Review, Claim Guard, Report Content Audit, Journey Assertion Audit, Assertion Suggestions, Business Journeys, Test Case Matrix, Test Data, Source Analysis, Source Runtime Correlation, Source Health, Artifact Integrity, Root Cause Groups, Issue Disposition, Fix Tasks, QA Gate, QA Sign-off, Professional Summary, Regression Plan, Risk Register, Risk Acceptance, Automation Specs, AI
...
`metadata.schemaVersion` is the machine-readable result contract version. Reports before `1.2.0` may miss journey/API/realtime/P2/fixTasks fields; reports before `1.3.0` may miss `qualityGate`; reports before `1.4.0` may miss `requirementCoverage`; reports before `1.5.0` may miss `artifactIntegrity`; reports before `1.6.0` may miss `rootCauseGroups`; reports before `1.7.0` may miss `issueDisposition`; reports before `1.8.0` may miss generated requirement-journey metadata; reports before `1.9.0` may miss `productContext`-aware disposition; reports before `1.10.0` may miss `sourceAnalysis`; reports before `1.11.0` may miss `sourceRuntimeCorrelation`; reports before `1.12.0` may miss `sourceHealth`; reports before `1.13.0` may miss `qaSignoff`; reports before `1.14.0` may miss `sourceHealth.scriptChecks[]`; reports before `1.15.0` may miss `environment`; reports before `1.16.0` may miss `pageProfile`; reports before `1.17.0` may miss `testData`; reports before `1.18.0` may miss `downloadPath` / `downloadSizeBytes` / `downloadSha256` and `artifacts.downloadedFiles[]`; reports before `1.19.0` may miss `downloadContent` parse summaries; reports before `1.20.0` may miss `regressionPlan` and `artifacts.regressionPlanLog`; reports before `1.21.0` may miss `professionalSummary` and `artifacts.professionalSummaryLog`; reports before `1.22.0` may miss pixel-level P2 visual diff fields such as `diffScreenshot`, `changedPixelCount`, `totalPixelCount`, `sizeMismatch`, and `diffBoundingBox`; reports before `1.23.0` may miss QA sign-off journey assertion counters and stricter recorded-journey business-validation downgrades; reports before `1.24.0` may miss `expectRequest` journey API assertions; reports before `1.25.0` may over-include conditional/non-actionable raw findings in `rootCauseGroups` and `fixTasks`; reports before `1.26.0` may miss `summary.adjustedScore` / `adjustedIssueCount` / `scoreNotes`; reports before `1.27.0` may miss `artifacts.evidenceReport` and may still use `report.md` as the full raw evidence report; reports before `1.28.0` may miss `scopeReview`, `artifacts.scopeReview`, and `artifacts.scopeReviewLog`; reports before `1.29.0` may miss `claimGuard`, `artifacts.claimGuard`, and `artifacts.claimGuardLog`; reports before `1.30.0` may miss `qaIntake`, `artifacts.qaIntake`, and `artifacts.qaIntakeLog`; reports before `1.31.0` may miss `defectProof`, `artifacts.defectProof`, and `artifacts.defectProofLog`; reports before `1.32.0` may include actionable-but-needs-evidence root causes in `fixTasks`, `professionalSummary.mustFix/shouldFix`, adjustedScore, qualityGate, or professional CI gates; reports before `1.33.0` may fail to retain reproducible exception no-feedback findings as frontend error-state root-cause candidates or may hide their EX/network/console/page-error evidence inside details; reports before `1.34.0` may miss `rootCauseGroups[].sourceLocations` and therefore require consumers to recover source file:line evidence from issue details, sourceAnalysis, or sourceRuntimeCorrelation manually; reports before `1.35.0` may have `report.md` / `report.html` generated from a pre-final artifactIntegrity snapshot even when `result.json.artifactIntegrity` is later corrected; reports before `1.36.0` may not roll medium/high `sourceRuntimeCorrelation.links[]` source matches into `rootCauseGroups[].sourceLocations` and may allow weak source-bound frontend root causes to become `probable`; reports before `1.37.0` may not include `sourceAnalysis.findings[kind=ui-accessibility]` or use it to bind runtime a11y button-name findings to source file:line; reports before `1.38.0` may not include `sourceAnalysis.findings[kind=error-state-gap]` or use it to bind exception no-feedback findings to views that track errors but render only empty states; reports before `1.39.0` only detect that pattern reliably in Vue templates, not Svelte or JSX/TSX render blocks; reports before `1.40.0` may miss multi-line source-template icon buttons without accessible names; reports before `1.41.0` may still include built-in heuristic AI summaries as raw `AI-001` issues; reports before `1.42.0` may emit source-aware API/UI list-empty mismatch guesses even when sourceRuntimeCorrelation was unavailable or weak; reports before `1.43.0` may emit raw issues for refresh/download/pagination warning-level interactions even when those capabilities were product-optional; reports before `1.44.0` may emit raw issues for small tap targets even when mobile/touch/WCAG scope was not explicit; reports before `1.45.0` may present raw score too prominently in human summaries; reports before `1.46.0` may over-promote optional SEO gaps or color-contrast findings without explicit public-content/SEO/WCAG/a11y scope; reports before `1.47.0` may include mismatched API/table/pagination fix suggestions on a11y/SEO/visual findings and should have those suggestions treated as template noise; reports before `1.48.0` may over-downgrade sourceAnalysis-confirmed eager route imports/heavy static imports as non-defect performance noise instead of proof-ready source-level should-fix candidates; reports before `1.49.0` may reference missing local screenshots/videos/downloads/DOM/visual artifacts without inline missing/unchecked markers, so consumers must cross-check artifactIntegrity before citing them; reports before `1.50.0` may only show non-defect bucket counts in `qa-review.md` without representative downgraded/non-fix issue samples and reasons; reports before `1.51.0` may not include `artifacts.professionalAudit`, `artifacts.professionalAuditLog`, `professional-audit.md`, or `professional-audit.json`, so consumers should run `frontlens audit --report` manually; reports before `1.52.0` may not include `artifacts.productContext`, `artifacts.productContextLog`, `product-context.md`, or `product-context.json`, so consumers should read `scopeReview.configSnippet` or run `frontlens product-context --report` manually; reports before `1.53.0` may not include `artifacts.productContextConfig` or `product-context.config.json`, so consumers should copy `usage.configSnippet` manually into a config before rerun; reports before `1.54.0` may not include `qaPlan`, `artifacts.qaPlan`, `artifacts.qaPlanLog`, `qa-plan.md`, or `qa-plan.json`, so consumers should combine professionalSummary + regressionPlan + qaIntake manually; reports before `1.55.0` may not include `qaCoverage`, `artifacts.qaCoverage`, `artifacts.qaCoverageLog`, `qa-coverage.md`, or `qa-coverage.json`, so consumers should manually inspect module sections and skipped checks; reports before `1.56.0` may not make professional-audit coverage-boundary blockers explicit, so consumers should compare qaCoverage with qaSignoff/claimGuard manually; reports before `1.57.0` may not include `report.profile`, so consumers should assume the normal professional report profile; reports before `1.58.0` may not include `reportContentAudit`, `artifacts.reportContentAudit`, `artifacts.reportContentAuditLog`, `report-content-audit.md`, or `report-content-audit.json`, so consumers must manually inspect generated Markdown for forbidden wording, raw evidence leakage, missing raw-score caveats, hidden coverage gaps, and artifact warnings; reports before `1.59.0` may not include `journeyAssertionAudit`, `artifacts.journeyAssertionAudit`, `artifacts.journeyAssertionAuditLog`, `journey-assertion-audit.md`, or `journey-assertion-audit.json`, so consumers must manually inspect `journeyTests[]` before treating click/fill replay as business validation; reports before `1.60.0` may not include professional-audit `disposition-quality` blockers, so consumers should manually verify that weak-evidence, product/style-scope, and API/UI mismatch guesses were not promoted to actionable fixes; reports before `1.61.0` may not warn when executive/professional `report.md` is too long or table-heavy, so consumers should manually prefer `brief.md` and `evidence-report.md` split when reports feel scanner-like; reports before `1.62.0` may not automatically add role-matrix follow-up items for credential/security pages, dangerous action buttons, permission warnings, or explicit role/auth requirements, so consumers should manually request multi-role storage states when permissions matter; reports before `1.63.0` may not automatically add source-health follow-up items for detected but unexecuted build/typecheck/test/e2e/lint scripts, so consumers should manually inspect `sourceHealth.packageScripts` versus `sourceHealth.scriptChecks` before professional sign-off; reports before `1.64.0` may miss `riskRegister`, `artifacts.riskRegister`, `artifacts.riskRegisterLog`, `risk-register.md`, and `risk-register.json`, so consumers should manually combine professionalSummary, qaSignoff, qaCoverage, qaPlan, sourceHealth, testData, artifactIntegrity, and environment into release risks; reports before `1.65.0` may miss `riskAcceptance`, `artifacts.riskAcceptance`, `artifacts.riskAcceptanceLog`, `risk-acceptance.md`, and `risk-acceptance.json`, so consumers should manually separate must-mitigate risks from accepted/acceptance-needed risks; reports before `1.66.0` may miss `testCases`, `artifacts.testCases`, `artifacts.testCasesLog`, `test-cases.md`, and `test-cases.json`, so consumers should manually combine requirementCoverage, journeyTests, interactionTests, exceptionSimulations, qaCoverage, sourceHealth, testData, and artifactIntegrity into a test execution ledger; reports before `1.67.0` may miss `assertionSuggestions`, `artifacts.assertionSuggestions`, `artifacts.assertionSuggestionsLog`, `assertion-suggestions.md`, and `assertion-suggestions.json`, so consumers should manually inspect journeyAssertionAudit path-only/weak journeys and propose concrete expect* steps; reports before `1.68.0` may default `report.md` to the fuller professional profile rather than the shortest executive decision brief; reports before `1.69.0` may not enforce the four-part API/UI data-mismatch proof gate, so consumers should manually require explicit requirement, exact list response, visible empty target UI, and source API/state/render binding before treating those findings as actionable; reports before `1.70.0` may still assign weak scope-sensitive empty-state/pagination observations to frontend owners, so consumers should reclassify them as QA/test evidence gaps unless PRD/productContext makes them required; reports before `1.71.0` may treat generic body text assertions such as `expectText body OK` as meaningful, so consumers should manually downgrade them to weak/path-only business evidence; reports before `1.72.0` may describe missing PRD/product scope as a generic pass-with-risks rather than an intake-first QA state, so consumers should manually lead with qaIntake/scopeReview before defect lists; reports before `1.73.0` may have a longer brief/executive report and looser compactness audit thresholds, so consumers should prefer brief.md and sidecar artifacts over pasted scanner detail; reports before `1.74.0` may not include `qa-intake.config.json`, so consumers should manually combine qaIntake questions, productContext, requirements, journeys, testData, safety, and source settings before rerun; reports before `1.75.0` may still emit API/UI data-mismatch guesses without an explicit provided requirement plus medium/high source-runtime binding, may not expose `qa-intake`, `risk-register`, `risk-acceptance`, or `artifact-integrity` helper commands, and may return raw unfiltered suggestions from CLI/MCP; reports before `1.76.0` may not expose `claim-guard`, `defect-proof`, `report-content-audit`, or `journey-assertion-audit` helper commands, so consumers may need to read the sidecar Markdown/JSON fields directly; reports before `1.77.0` may not include `_frontlensQaIntake.draftAssertionSteps[]` inside `qa-intake.config.json`, so consumers should copy from `assertion-suggestions.md/json` manually; reports before `1.78.0` may miss `defectTickets`, `artifacts.defectTickets`, `artifacts.defectTicketsLog`, `defect-tickets.md`, and `defect-tickets.json`, so consumers should derive bug-filing items from `defectProof proven|probable` + `rootCauseGroups` manually; reports before `1.79.0` may miss `traceability`, `artifacts.traceability`, `artifacts.traceabilityLog`, `traceability.md`, and `traceability.json`, so consumers should manually map requirements to test cases/defects/risks; reports before `1.80.0` may miss `automationSpecs`, `artifacts.automationSpecs`, `artifacts.automationSpecsLog`, `artifacts.automationSpecFile`, `automation-specs.md`, `automation-specs.json`, and `automation/frontlens.spec.ts`, so consumers should generate automation drafts from requirements/journeys/assertionSuggestions/testCases manually; reports before `1.81.0` may miss `evidenceBundle`, `artifacts.evidenceBundle`, `artifacts.evidenceBundleLog`, `evidence-bundle.md`, and `evidence-bundle.json`, so consumers should manually cross-check defect tickets/test cases/traceability/automation drafts against artifactIntegrity before sharing a handoff package; reports before `1.82.0` may miss `qaStrategy`, `artifacts.testStrategy`, `artifacts.testStrategyLog`, `test-strategy.md`, and `test-strategy.json`, so consumers should manually decide which modules are run, run-if-input, out-of-scope, or blocked; reports before `1.83.0` may miss `businessJourneys`, `artifacts.businessJourneys`, `artifacts.businessJourneysLog`, `business-journeys.md`, and `business-journeys.json`, so consumers should manually combine requirements, journeys, assertion suggestions, roles, and test-data gaps into a rerun scenario plan; reports before `1.84.0` may miss `reviewCalibration`, `review-calibration.md/json`, and `review-calibration.config.json`; reports before `1.85.0` may not preserve `reviewCalibration.calibrationSource=config` on reruns; reports before `1.86.0` may show an applied review-calibration contract in the report text without applying it to `issueDisposition`, adjusted score, fixTasks, or summaries, so consumers should manually enforce calibration decisions when reading those older reports. CLI/MCP helper commands normalize common missing sections to safe defaults, synthesize `fixTasks[]`, `qualityGate`, `qaSignoff`, `professionalSummary`, `regressionPlan`, `qaPlan`, `qaCoverage`, `assertionSuggestions`, `testCases`, `riskRegister`, `riskAcceptance`, `automationSpecs`, `evidenceBundle`, `qaStrategy`, `businessJourneys`, `requirementCoverage`, `rootCauseGroups[]`, and `issueDisposition` from normalized evidence, and expose safe defaults for `artifactIntegrity` / source correlation / source health / environment / pageProfile / scopeReview / claimGuard / qaIntake / defectProof / reportContentAudit / journeyAssertionAudit / testCases / testData / riskRegister / riskAcceptance / defectTickets when older reports do not contain them.
...
interface QaResult {
  ...
  businessJourneys: BusinessJourneyResult; // scenario planning pack; not pass evidence until rerun
  ...
  automationSpecs: AutomationSpecResult; // review-only Playwright regression drafts; not pass evidence until reviewed and executed
  evidenceBundle: EvidenceBundleResult; // shareable handoff index; do not cite missing-artifact refs as evidence
  qaStrategy: QaStrategyResult; // test strategy planner: run/run-if-input/out-of-scope/blocked decisions
...
interface ArtifactIndex {
  ...
  outputDir: string;
  markdownReport?: string; // default executive decision brief Markdown report
  ...
  professionalAudit?: string; // professional report-contract self-audit Markdown
  reportContentAudit?: string; // generated-report content audit Markdown
  journeyAssertionAudit?: string; // business-journey assertion quality audit Markdown
  assertionSuggestions?: string; // concrete expect* suggestion Markdown for weak/path-only journeys
  businessJourneys?: string; // business scenario plan Markdown for requirements/journeys/assertions/role/test-data gaps
  productContext?: string; // reviewable productContext suggestion Markdown
  productContextConfig?: string; // direct FrontLens config JSON containing the suggested productContext snippet
  qaIntakeConfig?: string; // reviewable FrontLens config pack for PRD/productContext/journey/testData/safety/source reruns
  qaPlan?: string; // professional QA execution/acceptance plan Markdown
  qaCoverage?: string; // professional QA coverage matrix Markdown
  testCases?: string; // formal professional test case matrix Markdown
  riskRegister?: string; // release risk register Markdown
  riskAcceptance?: string; // risk acceptance / must-mitigate decision checklist Markdown
  defectTickets?: string; // proof-ready bug-filing queue Markdown
  traceability?: string; // PRD-to-test-to-defect matrix Markdown
  automationSpecs?: string; // review-only automation draft Markdown
  automationSpecFile?: string; // generated Playwright spec draft
  evidenceBundle?: string; // shareable evidence handoff bundle Markdown
  testStrategy?: string; // QA strategy planner Markdown
...
`businessJourneys` is the professional business-scenario planning layer available in schema 1.83+ as `result.json.businessJourneys`, `business-journeys.md`, `business-journeys.json`, and `frontlens business-journeys --report`. It combines provided requirements, recorded/runtime journeys, assertion suggestions, source/runtime evidence refs, role needs, and test-data gaps into rerunnable scenarios. Treat it as a QA plan: `ready` means actions/assertions are available to rerun, not that the generated scenario has passed; `needs-input` and `manual-required` rows are requirement/journey/role/test-data follow-ups, not code defects.

```ts
type BusinessJourneySource = 'requirement' | 'journey' | 'assertion-suggestion' | 'page-model';
type BusinessJourneyStatus = 'ready' | 'needs-input' | 'manual-required' | 'skipped';

interface BusinessJourneyScenario {
  id: string;
  title: string;
...
interface BusinessJourneyResult {
  generatedAt: string;
...
`evidenceBundle` is the QA handoff manifest. It does not create new defects; it links already proof-aware artifacts and prevents sharing reports that cite missing local evidence. Treat `status=blocked` or any `missing-artifact` item as a packaging/evidence blocker before filing bugs or sending release notes.

```ts
interface EvidenceBundleResult {
  generatedAt: string;
  status: 'ready' | 'partial' | 'blocked' | 'empty';
  summary: { itemCount: number; readyCount: number; missingArtifactCount: number; defectTicketCount: number; failedOrBlockedTestCaseCount: number; traceabilityGapCount: number; automationDraftCount: number };
  artifactSummary: { presentCount: number; missingCount: number; skippedCount: number };
  items: Array<{
    id: string;
    kind: 'defect-ticket' | 'test-case' | 'traceability-gap' | 'automation-draft' | 'artifact';
    priority: 'P0' | 'P1' | 'P2' | 'P3';
    status: 'ready' | 'needs-input' | 'missing-artifact' | 'reference';
    owner: 'frontend' | 'backend' | 'product' | 'test' | 'security';
    title: string;
    evidenceRefs: string[];
    artifactRefs: Array<{ source: string; path: string; exists: boolean; sizeBytes?: number; message?: string }>;
    issueIds: string[];
    requirementIds: string[];
    testCaseIds: string[];
...
`qaStrategy` is the QA strategy planner. It marks each module as `run`, `run-if-input`, `out-of-scope`, `blocked`, or `already-covered`, lists the environment plan, and turns missing PRD/product scope/role/test-data/build-preview/source-script evidence into questions rather than speculative findings.

```ts
interface QaStrategyResult {
  generatedAt: string;
  ...
  summary: {
    ...
    recommendedRunMode: 'smoke' | 'focused' | 'full' | 'blocked';
    runCount: number;
  ...
  modules: Array<{ module: string; decision: string; priority: 'P0' | 'P1' | 'P2' | 'P3'; owner: string; reason: string; requiredInputs: string[]; evidenceRefs: string[]; commandHints: string[] }>;
  environments: Array<{ kind: 'dev' | 'preview' | 'staging' | 'production' | 'unknown'; purpose: string; trust: 'high' | 'medium' | 'low'; required: boolean; commandHint?: string }>;
  questions: Array<{ id: string; priority: 'P0' | 'P1' | 'P2' | 'P3'; category: string; question: string; reason: string; unblocks: string[] }>;
  nextCommands: string[];
  notes: string[];
}
```
...
`reportContentAudit` is generated from the final human-facing `report.md`. It is different from `professionalAudit`: `professionalAudit` validates the structured result contract, while `reportContentAudit` validates the generated Markdown wording and depth profile.

Use it before echoing a report conclusion. `status=failed` means the report text itself contains a blocker such as forbidden claim wording, raw evidence leaking into an executive/professional report, or a missing caveat that could mislead readers.

```ts
interface ReportContentAuditResult {
  status: 'passed' | 'warning' | 'failed' | 'skipped';
  checkedAt: string;
  profile: 'executive' | 'professional' | 'full';
  summary: {
    findingCount: number;
    blockerCount: number;
    warningCount: number;
...
## Journey Assertion Audit

`journeyAssertionAudit` is generated from `journeyTests[]`. It is the guardrail that prevents FrontLens or a downstream Agent from treating a passed click/fill replay as a validated business flow.

Use it before saying a business function passed. `runtime-verified` requires at least one meaningful passed `expectVisible`, `expectText`, `expectUrl`, or `expectRequest` assertion. `path-only` and `weakly-asserted` journeys are coverage evidence only; they require stronger assertions, PRD binding, role/test-data setup, or manual QA follow-up before acceptance.

```ts
interface JourneyAssertionAuditResult {
  status: 'passed' | 'warning' | 'failed' | 'skipped';
  checkedAt: string;
  summary: {
    journeyCount: number;
  ...
  items: Array<{
    ...
    meaningfulAssertionStepCount: number;
    assertionActions: Array<'expectVisible' | 'expectText' | 'expectUrl' | 'expectRequest'>;
...
interface JourneyAssertionAuditFinding {
  id: string;
  severity: 'blocker' | 'warning' | 'info';
  category: 'missing-assertion' | 'weak-assertion' | 'failed-assertion' | 'failed-journey' | 'requirement-binding';
  journeyId?: string;
  title: string;
  evidence: string;
  recommendation: string;
...
`assertionSuggestions` proposes concrete `expectVisible`, `expectText`, `expectUrl`, and `expectRequest` steps for path-only or weakly asserted journeys and uncovered provided requirements. They are improvement candidates, not passed evidence, until copied into requirements/journey config and rerun.

```ts
type AssertionSuggestionAction = 'expectVisible' | 'expectText' | 'expectUrl' | 'expectRequest';
type AssertionSuggestionSource = 'journey' | 'requirement' | 'page-model' | 'api';

interface AssertionSuggestionItem {
  id: string;
...
interface AssertionSuggestionResult {
  generatedAt: string;
  status: 'ready' | 'needs-input' | 'skipped';
  summary: { totalCount: number; journeySuggestionCount: number; requirementSuggestionCount: number; highConfidenceCount: number; weakJourneyCount: number; needsInputCount: number };
...
`testCases` is the formal execution ledger a professional tester expects alongside coverage and defects. It is generated from requirements, journey assertion quality, interaction probes, exception simulations, accessibility/responsive/performance/security checks, source health, test data, artifact integrity, and qaCoverage gaps. Use it to answer “what was actually tested, what passed, what failed, and what still needs input” without turning every coverage row into a code bug.

A `passed` test case is scoped evidence only; it does not override `claimGuard`, missing PRD, missing role/test-data coverage, or `riskAcceptance`. `needs-input` and `manual-required` rows are professional QA follow-ups unless runtime/source/requirement evidence makes them defects. `blocked` rows are sign-off blockers and professional CI blockers.
...
type TestCaseStatus = 'passed' | 'failed' | 'partial' | 'blocked' | 'skipped' | 'needs-input';
type TestCaseExecutionMode = 'runtime' | 'static' | 'hybrid' | 'manual-required';
...
interface TestCaseMatrixResult {
  generatedAt: string;
...
When `sourceRuntimeCorrelation.status=passed`, downstream triage should only treat API/UI data mismatch as a frontend defect when the relevant `networkRequestId` has `medium` or `high` correlation and the requirement/network/UI-empty/source four-part proof gate is complete. `high` means source API + UI component + state/list evidence aligned; `medium` means direct source API plus at least one runtime/UI/list signal aligned; `low` means only weak token matching and is not enough for a data-mismatch defect; `none` means the runtime response is not proven to feed the current page.
...
`productContextSuggestion` is the reusable version of the scope review. It is available as `product-context.md`, `product-context.json`, `product-context.config.json` (schema 1.53+ QA runs), and `frontlens product-context --report`. Use it after a noisy run to create a reviewable config snippet; after Product/QA confirms the answers, edit/approve `product-context.config.json` and rerun with `--config`, or copy `usage.configSnippet.productContext` into another FrontLens config, so style/product/device/a11y findings are downgraded consistently instead of re-argued by each Agent.

```ts
interface ProductContextSuggestionResult {
  generatedAt: string;
...
`defectTickets` is the human bug-filing queue available in schema 1.78+ as `result.json.defectTickets`, `defect-tickets.md`, `defect-tickets.json`, and `frontlens defect-tickets --report`. It contains only root causes whose `defectProof.status` is `proven` or `probable`; `needs-evidence`, product/design decisions, deployment-only work, tool limitations, and reference observations are suppressed. Use it for Jira/Linear filing; use `defectProof.missingEvidence` and `qaIntake.topQuestions[]` for anything not in this queue.

```ts
interface DefectTicketItem {
  id: string;
  rootCauseGroupId: string;
  proofStatus: 'proven' | 'probable';
  proofScore: number;
  confidence: 'high' | 'medium' | 'low';
  issueIds: string[];
  owner: 'frontend' | 'backend' | 'product' | 'test' | 'security';
  ...
  requirements: Array<{ id: string; title: string; priority: string; status: string }>;
  evidenceRefs: string[];
  artifactRefs: string[];
...
`professionalAudit` is the report-contract self-audit available as `professional-audit.md`, `professional-audit.json`, and `frontlens audit --report`. Read it before trusting a report produced by another Agent. A `failed` audit means the report is internally inconsistent, overclaims, or promotes weak/speculative findings: for example it schedules non-proof-ready/non-actionable items as fixes, allows a business-validation claim without requirements/assertions, overstates acceptance while qaCoverage has gaps, has failed artifact integrity, or lacks required source evidence for a frontend fix, or lets weak-evidence/product-scope/API-UI guesswork become actionable.

```ts
type ProfessionalAuditStatus = 'passed' | 'warning' | 'failed';
type ProfessionalAuditSeverity = 'blocker' | 'warning' | 'info';
type ProfessionalAuditCategory =
  | 'artifact-integrity'
  | 'report-content'
  | 'journey-assertion'
  | 'claim-guard'
...
interface ProfessionalAuditFinding {
  id: string;
...
Use `qaSignoff.status` for release/sign-off conversations and `qaSignoff.businessValidationConfidence` for business-function claims. A raw `qualityGate.pass` can still become `qaSignoff.pass-with-risks` when no PRD, login/role state, passed user journey, or passed journey assertion proves the business flow. A requirement-bound journey must have a meaningful business assertion before `businessValidationConfidence` can become `runtime-verified`; click/fill-only replay, body/html/#app visibility, or generic body text such as `OK`/`Done` remains runtime-partial and can block professional sign-off when tied to provided requirements.

`regressionPlan` is the machine-readable post-fix verification plan. It converts proof-ready root causes, defect-proof evidence gaps, failed requirements/journeys, source-health blockers or unexecuted project script gaps, download/export evidence, artifact integrity, environment trust, pageProfile/scopeReview/claimGuard/qaIntake gaps, and test-data gaps into rerun commands and focused verification items.
...
`qaPlan` is the professional tester worklist. It sits above raw issues and regression details: it tells the next tester/Agent which requirements, journeys, product-context decisions, environment checks, test data, source/CI script evidence, proof-ready fixes, and evidence gaps must be handled before acceptance or release claims.

```ts
interface QaExecutionPlanResult {
  generatedAt: string;
  status: 'ready' | 'needs-input' | 'blocked';
  confidence: 'high' | 'medium' | 'low';
  summary: string;
  ...
  items: Array<{
    id: string;
    type: 'rerun' | 'requirement' | 'journey' | 'root-cause' | 'defect-proof' | 'environment' | 'product-context' | 'test-data' | 'artifact-integrity' | 'download' | 'source-health' | 'role-matrix';
    priority: 'P0' | 'P1' | 'P2' | 'P3';
    owner: 'frontend' | 'backend' | 'product' | 'test' | 'security';
    status: 'ready' | 'needs-input' | 'blocked';
    title: string;
...
Use `qaPlan.items[]` as the default follow-up schedule. It is intentionally smaller and more acceptance-oriented than raw `issues[]`; keep raw details in `evidence-report.md` unless a plan item needs drill-down.

`qaCoverage` is the professional coverage matrix. It states which QA dimensions were actually covered, partially covered, skipped, need input, or failed. Use it before writing “已验证/通过”; skipped and needs-input rows are coverage gaps, not passes.

```ts
interface QaCoverageMatrixResult {
  generatedAt: string;
  ...
  items: Array<{
    id: string;
    area: 'runtime' | 'requirements' | 'journey' | 'interaction' | 'exception' | 'api-network' | 'source' | 'accessibility' | 'responsive' | 'performance' | 'security' | 'environment' | 'product-scope' | 'test-data' | 'artifact' | 'triage';
    title: string;
    status: 'covered' | 'partial' | 'skipped' | 'needs-input' | 'failed';
    confidence: 'high' | 'medium' | 'low';
    evidenceRefs: string[];
    covered: string[];
    gaps: string[];
...
Use `qaCoverage.items[]` to explain coverage limitations concisely. Do not expand every skipped selector or raw issue; report the affected area and the next input needed.

`riskRegister` is the release-risk matrix. It converts proof-ready defects, coverage gaps, environment/source/test-data/artifact issues, and sign-off blockers into impact × likelihood risks with owners, mitigations, and verification steps. Use it before release-style summaries or CI decisions; it is not a substitute for Product/QA risk acceptance.

```ts
interface RiskRegisterResult {
  generatedAt: string;
  ...
  items: Array<{
    id: string;
    category: 'defect' | 'coverage' | 'environment' | 'test-data' | 'source-health' | 'artifact' | 'product-scope' | 'permission' | 'release';
    title: string;
    impact: 'critical' | 'high' | 'medium' | 'low';
    likelihood: 'high' | 'medium' | 'low';
    exposure: number;
    level: 'critical' | 'high' | 'medium' | 'low';
    status: 'open' | 'blocked' | 'accepted' | 'mitigated';
    owner: 'frontend' | 'backend' | 'product' | 'test' | 'security';
...
If `riskRegister.status=blocked`, report the release-blocking risks before raw issue totals. If it is `at-risk`, state whether the risk is a proof-ready implementation defect, a coverage/input gap, an environment/source-health/test-data issue, or a product-scope decision awaiting acceptance.

`riskAcceptance` is the risk decision checklist generated from `riskRegister`. It does not auto-accept risk; it tells a QA/Release owner which items must be mitigated before release and which items need explicit Product/QA/Release acceptance if they remain open. Use it before release sign-off and CI decisions, especially when product/design/environment risks are intentionally accepted instead of fixed.

```ts
interface RiskAcceptanceResult {
  generatedAt: string;
  ...
  items: Array<{
    ...
    blocksRelease: boolean;
    owner: 'frontend' | 'backend' | 'product' | 'test' | 'security';
    decision: 'must-mitigate' | 'needs-acceptance' | 'accepted' | 'rejected' | 'deferred';
    requiredApprovers: Array<'product' | 'qa' | 'engineering' | 'security' | 'release-manager'>;
    acceptanceCriteria: string[];
...
If `riskAcceptance.status=blocked`, do not issue release sign-off until every `must-mitigate` item is mitigated and rerun evidence exists. If it is `needs-acceptance`, list the required approvers/evidence and keep the risk open until the decision is recorded.
...
node dist/cli.js claim-guard --report result.json
...
node dist/cli.js risk-register --report result.json
node dist/cli.js risk-acceptance --report result.json
node dist/cli.js artifact-integrity --report result.json
node dist/cli.js fix-tasks --report result.json
node dist/cli.js diff --before old/result.json --after new/result.json
node dist/cli.js suggestions --report result.json       # proof-aware default
node dist/cli.js suggestions --report result.json --all # raw audit mode
```
...
## automationSpecs / automation-specs.md/json

Schema 1.80+ adds `result.json.automationSpecs`, `automation-specs.md/json`, and `automation/frontlens.spec.ts`. These are review-only Playwright regression drafts generated from requirements, journeys, assertion suggestions, and formal test cases. `automationSpecs.summary.readyCount` means a draft contains executable selector/text/url assertions, not that it already passed. Count it as regression evidence only after QA reviews selectors, auth state, test data, destructive-action safety, and executes the generated/edited spec.
