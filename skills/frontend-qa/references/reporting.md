...
- reviewCalibration status, signal count, keep/downgrade/out-of-scope/needs-evidence counts, `review-calibration.md/json` and `review-calibration.config.json` path; use it to summarize human-feedback-calibrated non-defects instead of re-listing style/product/dev/data-mismatch raw findings;
...
- review calibration from `result.json.reviewCalibration`, `review-calibration.md/json`, and `review-calibration.config.json`; if feedback is provided, apply its issueDecisions before must-fix wording; if status is `needs-feedback`, ask for tester/Product input rather than guessing;
