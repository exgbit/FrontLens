...
默认 `frontend-qa` 禁止直接读取大型原始产物，例如 `result.json`、`network.json`、`page-model.json`、`evidence-report.md` 和大型 `report.md`；只读 `brief.md` / `qa-review.md` / compact helper 输出，需要专项取证时再调用 `frontend-qa-forensics`。
...
6. 默认只读取 `brief.md` / `qa-review.md` / compact helper 输出；禁止直接打开大型 `result.json`、`network.json`、`page-model.json`、`evidence-report.md` 或大型 `report.md`。
