...
- `professional-audit.md`: report-contract self-audit; flags overclaims, non-proof-ready fix queue entries, weak source evidence, scope gaps, and artifact integrity issues before another Agent trusts the report.
...
- `product-context.md`: reviewable suggested `productContext` config and scope questions; copy into config only after Product/QA confirms it, then rerun to reduce style/product false positives.
...
10. When product scope, ADRs, supported devices, or “this is designed this way” feedback is available, encode it in `productContext` before rerunning or triaging. Use `deviceScope`, `requiredFeatures`, `optionalFeatures`, `outOfScopeFeatures`, `decisions[]`, and `adrRefs[]` so style, export, pagination, refresh, and touch-target findings are classified by product scope rather than guesswork. If scope is missing, read `product-context.md` or run `frontlens product-context --report <result.json>` to get a reviewable config snippet and questions; do not treat the suggestion as confirmed until Product/QA accepts it.
...
7. Read outputs in this order: `brief.md` or `frontlens brief --report <result.json>` (fallback: `qa-review.md` / `professionalSummary`), then `professional-audit.md` or `frontlens audit --report <result.json>` for report-contract self-check, then `product-context.md` / `frontlens product-context --report <result.json>` for scope suggestions, then inspect `claimGuard`, `qaIntake`, `defectProof`, `issueDisposition`, `scopeReview`, `sourceAnalysis`, `sourceRuntimeCorrelation`, `sourceHealth`, `environment`, `artifactIntegrity`, and only then raw `evidence-report.md` details.
