...
- **代码负责取证与收敛**：浏览器自动化、截图、网络记录、异常模拟、Coverage、安全扫描、JSON/Markdown 报告生成，以及一页式 `brief.md`、`professional-audit.md`、`report.md` / `qa-review.md` 专业复盘摘要，并生成 `scope-review.md` / `product-context.md` 把产品/PRD 缺口转成可回答的问题和可复制配置草案，生成 `claim-guard.md` 防止业务通过/生产就绪等过度承诺。
...
- 生成 `scope-review.md` / `product-context.md`，把“样式是否要改 / 产品是否设计如此 / 页面是否该有分页导出”等不确定点变成产品范围问题和可复制的 `productContext` 配置草案
...
- 下载/导出文件保存与哈希校验
- 安全响应头和敏感信息扫描
- Markdown / JSON 报告产物（一页式 `brief.md` + 报告契约自检 `professional-audit.md/json` + 产品范围配置草案 `product-context.md/json` + 决策型 `report.md` + 原始证据 `evidence-report.md`）
- 面向修复排期的 `brief.md` / `professional-audit.md` / `product-context.md` / `report.md` / `qa-review.md` 精简专业复盘、`evidence-report.md` 原始证据附录与 `regression-plan.json` 回归复测清单
...
6. 优先读取 `brief.md`，再读取 `professional-audit.md` / `product-context.md` / `qa-review.md` / `report.md` / `result.json`，仅在需要钻取证据时读取 `evidence-report.md`。
...
如果某些能力是产品设计取舍，例如“PC 为主、移动端降级”“凭证页不允许导出”“当前页面不做分页”，优先读取 `product-context.md` 或运行 `frontlens product-context --report <result.json>` 获取草案；产品/QA 确认后，把它们写入配置的 `productContext`：
...
- 产品范围确认：`scope-review.md`、`product-context.md/json`、待回答问题、`productContext` 草案
...
`product-context` 会输出可复核的 `productContext` 配置草案与范围问题；产品/QA 确认后复制到配置并重跑，样式风格、分页/导出/刷新、移动触控等产品取舍项才会稳定降噪。
...
3. 证据采集引擎生成 raw artifacts、`professional-audit.md/json` 报告契约自检、`product-context.md/json` 产品范围草案、决策型 `report.md`、精简 `qa-review.md` 和完整 `evidence-report.md`。
4. LLM 优先读取一页式 `brief.md`，再按需读取 `professional-audit.md`、`product-context.md`、源码和规则文档。
