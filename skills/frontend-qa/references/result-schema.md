...
`sourceHealth` is a non-destructive source-health layer. It detects package-manager/scripts from `package.json` and parses TS/JS/Vue `<script>` blocks for syntax errors without running target-app scripts. It can prove build-blocking syntax problems, but it does not replace full project `typecheck`/`build`/`lint`.
...
If `sourceHealth.status=failed`, treat `frontend-source-health` issues as source-confirmed P1 build blockers. Still do not claim business validation passed solely because `sourceHealth.status=passed`; it only proves parsed source files had no syntax errors and lists scripts that can be used for deeper verification.
