...
`productContextSuggestion` is the reusable version of the scope review. It is available as `product-context.md`, `product-context.json`, and `frontlens product-context --report`. Use it after a noisy run to create a reviewable config snippet; after Product/QA confirms the answers, copy `usage.configSnippet.productContext` into the FrontLens config and rerun so style/product/device/a11y findings are downgraded consistently instead of re-argued by each Agent.
