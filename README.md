...
FrontLens 现已支持测试工程师需要的完整主流程：

```text
需求文档 → 结构化需求 → 前端/后端/API/代码测点 → 分级测试用例
→ 开发 P0 阻塞自测 → 页面/API/代码执行 → 缺陷复现 → Markdown 报告
```

```bash
# 1. 生成完整测试计划、开发 P0 用例和 QA 全量用例
node dist/cli.js test-plan --input examples/demo-prd.md --source-root examples/demo-app --output reports/demo-plan

# 2. 执行页面、API 和代码侧测试
node dist/cli.js qa --url http://127.0.0.1:4173 --config examples/demo-app/frontlens.config.json --requirements reports/demo-plan/test-plan.json --source-root examples/demo-app --source-run-scripts --source-scripts "typecheck,lint,test" --output reports/demo-qa --sme

# 3. 将计划与实际证据合并为最终测试报告
node dist/cli.js test-report --plan reports/demo-plan/test-plan.json --report reports/demo-qa/result.json --output reports/demo-final
```

Agent/MCP 默认使用低 Token 摘要：需求提取、测试计划和执行报告均返回有界样本，并优先读取 `test-plan-summary.json`、`brief.md`、`test-execution-summary.json` 和精简后的 `test-report.md`。完整 `test-plan.json`、`result.json`、`test-execution-report.json` 与 `test-execution-details.md` 仅按需读取；CLI 的 `--json`、MCP 的 `detail=true/includeMarkdown=true` 会显式返回大体积明细。

Library API 提供同一条链路：`synthesizeRequirements` → `buildTestPlan` → `runQa` → `buildTestPlanExecutionReport`。面向 Agent 时应对前三类结构化结果分别使用 `compactRequirementWizard`、`compactTestPlan` 和 `compactTestPlanExecution`，不要默认把完整计划或完整执行账本送入模型上下文。相关输入类型 `RequirementWizardInput`、`BuildTestPlanInput` 及全部结果类型均由包根入口导出。

后端单测需要在目标项目的 `.frontlens/test-evidence.json` 中显式绑定 `requirementIds + layer + scenarios + scriptNames`；全局测试通过不会批量冒充需求通过。普通 `qa` 会记录 P0 阻塞但不会仅因计划待评审而退出非零；CI 需要把 P0 未关闭作为门禁时显式加 `--fail-on-blocked`。

API 证据按 **HTTP method + 模板路径 + PRD 明确状态码** 归因：例如 `DELETE /api/users/{id} 返回 403` 不会与同路径 GET 混用，也不会把验收所要求的 403 误报为接口失败；异常模拟同样必须匹配 HTTP method 才能关闭需求级 recovery 用例。一条需求包含多个 API 时，每个 method/path 会独立绑定自己的预期状态码，不允许 GET 200 和 DELETE 403 互换证据。

### 输出产物与状态语义

| 产物 | 用途 | Agent 默认读取 |
|---|---|---|
| `test-plan-summary.json` | 需求、测点、P0 和阻塞覆盖摘要 | 是 |
| `developer-test-cases.md` | 开发提测前必须执行的 P0 子集 | 是 |
| `qa-full-test-cases.md` | 测试工程师的全量分层用例 | 按需 |
| `test-report.md` | 低 Token 决策摘要 | 是 |
| `test-execution-details.md` | 逐条用例实际结果、证据和问题 | 按需 |
| `test-execution-report.json` | 完整机器可读执行账本 | 否 |

- `passed`：计划范围内的独立分层证据已通过。
- `partial`：已有部分通过证据，但仍有部分、跳过或未执行用例；不代表实现缺失。
- `needs-review`：PRD 草案、角色、写操作或验收标准需人工确认；该状态本身不等于执行阻塞。
- `blocked`：P0 未关闭、全局代码健康失败或必要环境/证据不可用。

项目回归基线（2026-07-13）：`npm run check` 通过、`npm test` 257/257 通过、`npm run build` 通过；Demo 生成 11 条需求、31 个测点、149 条 QA 用例和 21 条开发 P0 用例，阻塞类别遗漏为 0。

详细步骤见：[需求驱动测试操作指南](docs/requirement-driven-testing-guide.md)。
