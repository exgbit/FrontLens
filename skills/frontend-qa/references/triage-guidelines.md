...
Report raw score separately from adjusted risk and `riskRegister.status`. If many findings are skipped, synthetic, or deployment-only, say the score is low-confidence and prioritize the triaged fix list instead of repeating every issue.
...
1. Professional headline: QA sign-off, riskRegister status/release-blocking count, adjusted score, proof-ready root-cause count, and confidence.
