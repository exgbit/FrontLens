...
- 安全响应头和敏感信息扫描
- Markdown / JSON 报告产物
- 面向修复排期的 `qa-review.md` 精简专业复盘与 `regression-plan.json` 回归复测清单
...
注意：录制出的点击/填写步骤只能证明路径可执行。要达到业务验收级别，需要在 JSON 中补 `expectVisible`、`expectText`、`expectUrl`，并为新增/编辑/删除/上传/提交等写操作配置 `testData`、setup/cleanup 和授权环境。密码、token、secret、凭证等敏感输入会自动写成 `<REDACTED>`，回放前必须替换为隔离测试数据。危险按钮默认不会标记 `allowMutating=true`，除非录制时显式加 `--allow-mutating-steps`。
