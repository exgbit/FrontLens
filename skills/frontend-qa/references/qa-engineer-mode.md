...
- **Review calibration**: inspect `reviewCalibration.status`, `signals[]`, `issueDecisions[]`, and `configPatch`. If it is `ready`, apply keep/downgrade/out-of-scope/needs-evidence/ask-product decisions before the fix list; if it is `needs-feedback`, ask for tester/Product feedback rather than inventing product intent.
