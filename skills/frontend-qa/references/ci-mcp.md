...
`frontlens_qa` and `frontlens_inspect` include `qaSignoff` and `qualityGate` with `status` (`pass`, `pass-with-risks`, `fail`, `blocked`) and `confidence`, plus `requirementCoverage` summary/details, `sourceHealth`, `artifactIntegrity`, `issueDisposition`, and `rootCauseGroups`; use `qaSignoff` as the first machine-readable release/sign-off gate before applying source/requirement triage.
